...
- (void)handleUIApplicationWillChangeStatusBarFrameNotification:(NSNotification*)notification
{
    ...
    CGFloat OffsetY = 0;//CGRectGetHeight(newStatusBarFrame) - 20;
...
- (void)shareFaceView{
    ...
    if (!self.faceView) {
        ...
        if (chatKeyboardData.faceViewBgColor) {
            self.faceView.backgroundColor = chatKeyboardData.faceViewBgColor;
        }
...
- (void)shareShareMeun
{
    if (!self.shareMenuView)
    {
        ...
        ChatKeyboardData *chatKeyboardData = [ChatKeyboardData sharedChatKeyboardData];
        if (chatKeyboardData.shareViewBgColor) {
            self.shareMenuView.backgroundColor = chatKeyboardData.shareViewBgColor;
        }
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        switch (state) {
            ...
            case ZBMessageViewStateShowFace:
            {
                ...
                self.faceView.frame = CGRectMake(0.0f, UEX_SCREENHEIGHT - CGRectGetHeight(rect) - tmpY, UEX_SCREENWIDTH, CGRectGetHeight(rect));
            ...
            {
                self.shareMenuView.frame = CGRectMake(0.0f,UEX_SCREENHEIGHT-CGRectGetHeight(rect) - tmpY,UEX_SCREENWIDTH,CGRectGetHeight(rect));
